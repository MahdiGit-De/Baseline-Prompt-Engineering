...
## Purpose
This prompt extracts and structures publicly available evidence about a startup, classifying information into current facts, historical facts, inferences, assumptions, unknowns, conflicts, and source quality notes. It builds a rigorous evidence base without drawing conclusions or evaluations.

## When to Use
Use this prompt at the beginning of the evaluation workflow to gather and organize all relevant public information about a startup. It serves as the foundation for subsequent analysis in Prompt 2 and Prompt 3.
...
## Usage Instructions
1. Replace "Mutable" with the target startup name and update the evaluation date as needed.
2. Provide the prompt to an AI model (e.g., in VS Code with GitHub Copilot or ChatGPT).
3. The output will be a structured evidence base ready for Prompt 2 evaluation.
4. Save the output in the `/datasets` folder as `prompt-1-output-[startup].md`.

## Notes for Improvement
- Add support for automated web scraping to reduce manual research time.
- Include templates for different industry verticals (e.g., SaaS vs. hardware).
- Enhance citation formatting for better traceability.